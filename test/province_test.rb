...
class ProvinceTest < Minitest::Test
  ...
  def test_province_profit_with_no_producers
  	no_producers = Province.new(sample_province_data_no_producers)
  	assert_equal(0, no_producers.profit)
  end

  def test_province_shortfall_with_0_demand
  	asia = Province.new(sample_province_data)
  	asia.demand = 0
  	assert_equal(-25, asia.shortfall)
  end

  def test_province_profit_with_0_demand
  	asia = Province.new(sample_province_data)
  	asia.demand = 0
  	assert_equal(0, asia.profit)
  end

  def test_province_shortfall_with_negative_demand
  	asia = Province.new(sample_province_data)
  	asia.demand = -1
  	assert_equal(-26, asia.shortfall)
  end

  def test_province_profit_with_negative_demand
  	asia = Province.new(sample_province_data)
  	asia.demand = -1
